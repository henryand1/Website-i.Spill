...
<section class="sub-page product">
  <div class="container1"> 
      <div class="row2" style="justify-content: left;">
          <div class="prod2">
          <div class="content1">
            <img src="{{ url('foto_produk/'.$powder->image) }}" style="border-radius: 15px 15px;
            width: 333px;
            margin-top: 40px;">
          </div>
          </div>
          ...
          margin-top: -276px;">
              <div>
                  <ul style="text-align: left;">
                      ...
                      font-size: 30px;
                      margin-bottom: 15px;">{{$powder->brand}}</div>
                      <div id="nama_produk" style="    font-size: 45px;
                      color: dimgrey;
                  }">{{$powder->nama_produk}}</div>
                      <div style="font-size: 25px;
                      margin-top: 10px;">Rp. {{$powder->harga}}</div>
                  </ul>
              </div>
              </div>
      </div>
...
<section class="sub-page" style="margin-top: 203px;font-size:50px">
  ...
  <p style="margin-bottom: 200px;font-size:18px;margin-top:20px">{{$powder->keterangan}}</p>
</section>
...
<section class="sub-page" style="height: auto;">
  ...
  @Auth  
  <form action="insertreviewpowder/{{$powder->no_produk}}/{{$powder->nama_produk}}/{{Auth::user()->name}}" method="POST" enctype="multipart/form-data" 
    style="width: 700px">
    @csrf
    @else
    <a href={{route('login')}}> 
...
<section style="margin-top: 100px;">
 @foreach ($reviewpowder as $contoh)
